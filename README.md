...
| `11_latency_and_baselines/` | Batch-one latency distributions for GPU/CPU + optimized ONNX Runtime CPU baseline and optimized GPU-side baselines (ONNX Runtime CUDA, PyTorch FP16). |
...
| `14_sota_rerun/` | Same-protocol reruns of DeepLOB / TLOB / MLP / LSTM (Macro-F1, full training history, per-class metrics, confusion matrices) plus the test-set access audit; source of the comparable zone of the positioning table . |
...
- **Power / energy efficiency:** deployed LO-Arch whole-board power is directly
  ...
  5.98–10.44× latency reduction and 23.8–41.6× energy-efficiency improvement over
  the RTX 5070 Laptop GPU baseline, the range spanning an optimized ONNX Runtime
  CUDA engine (341 µs) and an eager-PyTorch baseline (594.96 µs) at the same FP32
  precision. The chip-level values in `8_postroute_reports/` are
